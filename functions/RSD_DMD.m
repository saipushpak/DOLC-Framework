function K_RSD_DMD = RSD_DMD(Xp,Xf,LAMBDA, N, Tpp, Tfp, Rpp, T1, T2)
...
for k1 = 1:N
    % Initialization with respect to the data (noticed that this results in an unstable KO)
    %     phi_T1{k1}     = Rpp{k1}*Tpp{k1}*Xp(:,1:T1)*(Rpp{k1}*Tpp{k1}*Xp(:,1:T1))';
    %     varphi_T1{k1}  = Tfp{k1}*Xf(:,1:T1)*(Rpp{k1}*Tpp{k1}*Xp(:,1:T1))';
    %     inv_phi_T1{k1} = pinv(phi_T1{k1});
    
    % Random initialization 
    phi_T1{k1}     = 0.01*eye(size(Rpp{k1},1)); 
    varphi_T1{k1}  = zeros(size(Tfp{k1},1),size(Rpp{k1},1));
    inv_phi_T1{k1} = pinv(phi_T1{k1}+LAMBDA*eye(size(phi_T1{k1})));
    
    % Koopman row blocks computation
    Kp_T1{k1}      = varphi_T1{k1}*inv_phi_T1{k1}*Rpp{k1};
end

% RSD-DMD at the starting time-point
k3 = 1;
K_RSD_DMD{k3} = cell2mat(Kp_T1);
...
for k2 = T1:T2
    ...
    for k1 = 1:N % for every agent
        ...
        Kp_T2{k1}         = varphi_T2{k1}*inv_phi_T2{k1}*Rpp{k1};
